...
let make = (
  ...
  | SignMetaTx =>
    <React.Fragment>
      <Rimble.Heading>
        {"Please sign the message to submit this transaction."->React.string}
        <WildcardsLoader />
      </Rimble.Heading>
      <Rimble.Flex justifyContent="center">
        <Rimble.Loader size="80px" />
  ...
  | SubmittedMetaTx =>
    <React.Fragment>
      <Rimble.Heading>
        {"Transaction Submitted "->React.string}
        <WildcardsLoader />
      </Rimble.Heading>
  ...
  | ContractActions.SignedAndSubmitted(_txHash) =>
    <React.Fragment>
      ...
      <Rimble.Text>
        <a href={`https://$txExplererUrl/tx/$txHash`} target="_blank" rel="noopener noreferrer">
          {("View the transaction on " ++ txExplererUrl)->React.string}
        </a>
      </Rimble.Text>
      <Rimble.Loader className=Styles.centerItems size="80px" />
    </React.Fragment>
  | ContractActions.Complete(_result) =>
    <React.Fragment>
      <Rimble.Heading>
