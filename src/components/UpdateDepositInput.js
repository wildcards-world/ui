...
export default ({ depositChange, updateDepositChange, isAddDeposit, updateIsAddDeposit, onSubmitDepositChange }) =>
  <Box p={4} mb={3} >
    ...
    <Radio checked={isAddDeposit} onChange={() => updateIsAddDeposit(true)} label="Deposit" my={2} />
    <Radio checked={!isAddDeposit} onChange={() => updateIsAddDeposit(false)} label="Withdrawl" my={2} />
    <Input
      type='number'
      name='value'
      value={depositChange}
